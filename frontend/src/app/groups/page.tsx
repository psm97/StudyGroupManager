  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setMyGroups(my.length ? my : [
        {id:1, name:'Web Developer Study', member_count:6, role:'leader', attendance_rate:92, color:'#1258fc'},
              ...
              <div className="rounded-2xl p-5 sm:p-6 text-white" style={{background:'linear-gradient(135deg,#0d52f3 0%,#286af8 55%,#3a74ef 100%)'}}>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <button className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white text-sm font-bold shadow hover:bg-blue-50 transition-all" style={{color:'#1258fc'}}>
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="flex border-b border-slate-100">
                  <button id="tab-my"
                    ...
                    내 그룹 <span className="ml-1.5 badge" style={{background:'#dce6fd',color:'#1258fc'}}>{myGroups.length}</span>
                  </button>
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold flex-shrink-0"
                            style={{background:g.color||'#1258fc'}}>{g.name[0]}</div>
                          <div>
                        ...
                        {g.role && (
                          <span className="badge flex-shrink-0" style={{background: g.role==='leader'?'#dce6fd':'#f1f5f9', color: g.role==='leader'?'#1258fc':'#64748b'}}>
                            {g.role==='leader'?'리더':'멤버'}
                          </span>
                        )}
                      </div>
                      {g.attendance_rate !== undefined && (
                        <div>
                          <div className="flex justify-between text-xs mb-1">
                            ...
                            <span className="font-semibold" style={{color:g.color||'#1258fc'}}>{g.attendance_rate}%</span>
                          </div>
                          <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div className="progress-bar h-full rounded-full" style={{width:`${g.attendance_rate}%`, background:g.color||'#1258fc'}}></div>
                          </div>
                        </div>
                      )}
                    </a>
