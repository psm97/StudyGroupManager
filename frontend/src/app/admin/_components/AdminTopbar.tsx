...
export default function AdminTopbar() {
  ...
  const [adminInfo, setAdminInfo] = useState({ username: '관리자', email: '' });

  useEffect(() => {
    fetch('/admin/api/me/', { credentials: 'include' })
      .then(r => r.ok ? r.json() : null)
      .then(data => { if (data) setAdminInfo({ username: data.nickname || data.username || '관리자', email: data.email || '' }); })
      .catch(() => {});
  }, []);
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        <div className="relative">
          <button
            ...
            <span className="hidden sm:block text-sm font-semibold text-slate-700">{adminInfo.username}</span>
          ...
          {profileMenuOpen && (
            <div className="absolute right-0 top-12 w-52 rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50" style={{background:'#fff'}}>
              <div className="px-4 py-3 border-b border-slate-100">
                <p className="text-sm font-bold text-slate-800">{adminInfo.username}</p>
                <p className="text-xs text-slate-400 truncate">{adminInfo.email || '관리자 계정'}</p>
              </div>
              ...
              <div className="py-1.5 border-t border-slate-100">
                <button
                   ...
                   className="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-red-500 hover:bg-red-50 transition-colors font-medium text-left">
