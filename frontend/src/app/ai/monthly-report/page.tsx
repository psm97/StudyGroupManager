      ...
      <div className="max-w-[1440px] mx-auto my-0 lg:my-8 bg-white lg:rounded-[32px] shadow-2xl flex overflow-hidden" style={{minHeight:'100vh'}}>
        ...
        <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto bg-slate-50 px-4 lg:px-8 py-5 lg:py-6">
            ...
            {/* 개인 기준 뷰 */}
            {(!isLeader || viewMode === 'personal') && (
              <div className="space-y-5">
                {myReport ? (
                  <>
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 sm:p-6" style={{boxShadow:'0 4px 24px rgba(0,0,0,.06)'}}>
                      <h2 className="font-bold text-slate-800 text-base mb-4 flex items-center gap-2">
                        ...
                        내 {myReport.report_month}월 출석 현황
                      </h2>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                        <div className="bg-blue-50 rounded-xl p-4 text-center border border-blue-100">
                          <p className="text-xs text-blue-400 font-semibold mb-1">내 출석률</p>
                          <p className="text-2xl font-bold" style={{color:'#0077ff'}}>{myReport.attendance_rate}%</p>
                        </div>
                        <div className="bg-slate-50 rounded-xl p-4 text-center border border-slate-100">
                          <p className="text-xs text-slate-400 font-semibold mb-1">총 세션</p>
                          <p className="text-2xl font-bold text-slate-700">{myReport.session_count}회</p>
                        </div>
                        <div className="bg-amber-50 rounded-xl p-4 text-center border border-amber-100">
                          <p className="text-xs text-amber-400 font-semibold mb-1">지각</p>
                          <p className="text-2xl font-bold text-amber-700">{myReport.late_count}회</p>
                        </div>
                        <div className="bg-red-50 rounded-xl p-4 text-center border border-red-100">
                          <p className="text-xs text-red-400 font-semibold mb-1">결석</p>
                          <p className="text-2xl font-bold text-red-600">{myReport.absent_count}회</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div className="bg-red-50 rounded-xl p-4 border border-red-100">
                          ...
                          <p className="text-xl font-bold text-red-600">{myReport.penalty_total.toLocaleString()}원</p>
                        </div>
                        <div className="bg-green-50 rounded-xl p-4 border border-green-100">
                          <p className="text-xs text-green-400 font-semibold mb-1">납부 완료</p>
                          <p className="text-xl font-bold text-green-600">{myReport.penalty_paid.toLocaleString()}원</p>
                        </div>
                        <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                          <p className="text-xs text-slate-400 font-semibold mb-1">미납</p>
                          <p className="text-xl font-bold text-slate-700">{myReport.penalty_unpaid.toLocaleString()}원</p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center" style={{boxShadow:'0 4px 24px rgba(0,0,0,.06)'}}>
                    ...
                    <h3 className="font-bold text-slate-700 text-lg mb-2">아직 생성된 개인 리포트가 없습니다</h3>
                    <p className="text-slate-400 text-sm">매월 말일 자동으로 생성됩니다.</p>
                  </div>
                )}
              </div>
            )}
